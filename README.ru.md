...
<p align="center">
  ...
  <img alt="Платформа Windows, macOS в планах" src="https://img.shields.io/badge/platform-Windows%20%C2%B7%20macOS%20planned-91a3b3?style=flat-square">
...
| FST | **16 ГБ** | 25 с | **ни память, ни время** |
...
**FST задаёт нижнюю планку, и длина трека ему безразлична.** Он всегда обрабатывает 48
десятисекундных сегментов: короткий файл дополняется до 48, а не считается меньшим их числом.
Файл на 32 секунды и файл на восемь минут стоят одинаковых 16 ГБ — это замер, а не оценка. Так
батчит upstream, и снаружи этот рычаг не повернуть. Практический вывод: **карта меньше 24 ГБ
потянет здесь всё, кроме FST**, и 12 ГБ — вполне рабочая машина для lofcz, артефактных метрик,
карты по времени и экспорта в MIDI.
...
**Настройка каждого детектора в интерфейсе.** Пороги, длины окон и сегментов, выбор устройства
сейчас живут в дефолтах адаптеров. Им место в панели настроек — сохранёнными отдельно по каждому
детектору и записанными в запуск: сохранённая оценка всегда должна уметь сказать, при каких
настройках она получена.

**macOS.** В планах, не в обещаниях, и упирается в железо — об этом ниже.
...
скрипты заточены под PowerShell и Conda. Всё работает локально, аудио никуда не загружается.

**macOS в планах**, и тут стоит быть точным. Нужны две вещи. Адаптеры просят CUDA по имени —
FST без неё вообще отказывается стартовать, — так что порт означает ветку MPS в каждом адаптере
и PowerShell-скрипты в переносимом виде. Сложнее вторая половина: у Apple Silicon память общая
между CPU и GPU, поэтому фиксированные 16 ГБ рабочего набора FST — это 16 ГБ всего, что есть у
машины. MacBook с 16 ГБ этот детектор не потянет, реальная планка для него — 32 ГБ, тогда как
всему остальному в лаборатории хватает заметно меньшего. Сделаю, когда здесь появится Mac, на
котором это можно проверить: порт, который никто не запускал, — это не поддержка.
